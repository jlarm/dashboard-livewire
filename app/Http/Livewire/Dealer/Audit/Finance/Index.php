...
class Index extends Component
{
    ...
    {
        $this->store = Store::with('glbaViolationAudits')->where('id', resolve('currentStore'))->firstOrFail();
    }
    ...
    public function render(): View
    {
        ...
        $chartGradesNumeric = $chartData->map(fn ($item): int => $gradeMap[mb_strtoupper((string) $item['grade'])] ?? 0)->all();
        $chartGradesLetters = $chartData->map(fn ($item): string => mb_strtoupper((string) $item['grade']))->all();
